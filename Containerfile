...
RUN --mount=type=tmpfs,dst=/tmp --mount=type=tmpfs,dst=/usr/lib/sysimage/cache/pacman pacman -Sy sbsigntools systemd-ukify kmod mokutil linux-hardened podman git base base-devel systemd  networkmanager nano podman plasma-desktop --needed --noconfirm
RUN --network=none \
